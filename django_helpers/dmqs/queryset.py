...
from django_helpers.dmqs.foundation import evaluate_condition, find_groups, mixed_sort

from django_helpers.dmqs.aggregation import Count

repository = Repository()


def universal_getters(*names):
    def one_get(obj, _name):
        if hasattr(obj, _name):
            return getattr(obj, _name)
        elif _name in obj:
            return obj[_name]
        else:
            raise AttributeError(f'{obj.__class__.__name__} hasn`t attribute {_name}')
    def getter(obj):
        if len(names) == 1:
            return one_get(obj, names[0])
        else:
            return (one_get(obj, name) for name in names)
...
class MemoryQuerySet(object):
    ...
    def order_by(self, *args):
        ...
        if not any(reverses):
            if self.use_my_getter:
                data = sorted(data, key=universal_getters(*properties))
            else:
                data = sorted(data, key=attrgetter(*properties))
        # all True (-age, -birthday)
        elif all(reverses):
            if self.use_my_getter:
                data = sorted(data, key=universal_getters(*properties), reverse=True)
            else:
